...
- The newer NDK's have a more aggressive Fortify level.  One JNI function was changed to increase safety without changing function, thus avoiding the Fortify crashes without having to disable or weaken Fortify checks on the datbase.cpp JNI code. So kmp-android-jni needs to be newly published to mavenLocal.
...
- JUnit 6.0.2 used for android unit tests
  - de.mannodermaus plugin 2.0.1 used to enable JUnit5/6 for android device tests
  - to run android instrumented tests, androidMinSdk must be set to 35 or desguraing errors occur. androidMinSdk is still 26 for publishing. 
- renamed the main module directory from KmpSqlencrypt to kmp-sqlencrypt to match the publish name to avoid issue with unit test runs.
...
- Switched to Vanniktech plugin for publishing. This required changing the gradle root project and project names to match the publishing artifact ID to work around a Vanniktech plugin issue.
